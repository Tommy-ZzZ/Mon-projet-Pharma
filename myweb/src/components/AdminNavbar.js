...
const AdminNavbar = ({ onToggleSidebar, onLogout, title }) => {
  ...
  const handleNotificationClick = async (notificationId) => {
    ...
    // Effectuer la mise à jour côté serveur pour marquer comme lue
    const response = await fetch(`http://192.168.209.25:4000/api/notification/${notificationId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ lue: true }), // Envoyer l'état lue=true
    });

    if (response.ok) {
      // Mettre à jour les notifications en local après la mise à jour côté serveur
